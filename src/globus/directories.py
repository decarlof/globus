...
def make_directory_name(args):
    ...
    '''
    # return '{0:s}-{1:s}'.format(args.year_month, args.pi_last_name)
    return '{0:s}-{1:s}-{2:s}'.format(args.year_month, args.pi_last_name, args.gup_number)
...
def create_analysis_dir_name(args):
    ...
    # analysis_path = Path(args.analysis_top_dir).joinpath(args.year_month,exp_name)
